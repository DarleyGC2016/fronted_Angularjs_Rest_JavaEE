angular
  ...
  .controller("PessoaListarController", PessoaListarController);
...
function PessoaListarController(
  ...
  vm.init = function() {
    HackatonStefaniniService.listar(vm.url).then(function(responsePessoas) {
      if (responsePessoas.data !== undefined)
        vm.listaPessoas = responsePessoas.data;

      vm.listaPessoasMostrar = [];
      var max =
        vm.listaPessoas.length > vm.qdePorPagina
          ? vm.qdePorPagina
          : vm.listaPessoas.length;

      vm.qdePaginacao = new Array(
        vm.listaPessoas.length % vm.qdePorPagina === 0
          ? vm.listaPessoas.length / vm.qdePorPagina
          : parseInt(vm.listaPessoas.length / vm.qdePorPagina) + 1
      );
      vm.currentPage = 1;
      ...
      HackatonStefaniniService.listar(vm.urlEndereco).then(function(
        responseEndereco
      ) {
  ...
  vm.remover = function(id) {
    ...
    angular.forEach(vm.listaEndereco, function(value, key) {
      console.log(value);
      if (value.id === id) liberaExclusao = false;
    });
